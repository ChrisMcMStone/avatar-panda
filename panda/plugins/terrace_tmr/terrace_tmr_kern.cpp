...
std::map<target_ulong,uint8_t> syscall_writemap;
//int syscall_num = 0;
...
{
    //No need to log kernel reads
    if (kernel_tracing) return 0;
...
int kernel_mem_write_cb(CPUState *env, target_ulong pc, target_ulong addr, target_ulong size, void *buf) {

    //Only log if we are in kernel
    if (!kernel_tracing) return 0;
    return 0;
    ...
    //            [address](mem_range_t m) {return ((m.addr <= address) && (address <= m.addr + m.size));});
        ...
        // Equivalent to checking memory in userspace
    //    if (mem_range == memory_ranges.end() || mem_range->flags & IS_ROM) continue; 

        //TODO create new map here

        // Don't need to check if already logged, as we want to allow overwrites. 
        //syscall_writemap[address] = val;
  //  }
}
...
int before_block_exec_trace_tb(CPUState *env, TranslationBlock *tb) {
    if (panda_in_kernel(env)) {
        kernel_tracing = true;
    } else if (kernel_tracing) { // we have just left kernel trace
        //TODO write TMRSyscall
...
bool init_plugin(void *self)
{
    ...
    kmem_file_name = panda_parse_string_opt(args, "kernel_syscall_memory_file",
            "kernel_writes.bin", "File storing memory written by systemcalls");
    ...
    pcb.virt_mem_after_write = kernel_mem_write_cb;
