...
int before_block_exec_trace_tb(CPUState *env, TranslationBlock *tb) {
    ...
    if( asid == 0 || panda_current_asid(env) != asid ) return 0;

    if ( (pc != last_pc || n == UINT_MAX ) && n > 0) {
        if(log_file.is_open())
            log_file << std::hex << last_pc << std::dec << "    " << n << std::endl;
...
bool init_plugin(void *self) {
    ...
    const char* log_file_name = panda_parse_string_opt(args, "log_file", "", "known start/breakpoint address in record");
